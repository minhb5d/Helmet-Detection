...
class RegionProposalNetwork(nn.Module):
    ...
    def assign_targets_to_anchors(self, anchors, gt_boxes):
        ...
        if gt_boxes.numel() == 0:
          ...
          matched_gt_boxes = torch.zeros((anchors.shape[0], 4), dtype=torch.float32, device=anchors.device)
    ...
    def forward(self, image, feat, target=None):
        ...
        if not self.training or target is None:
            ...
            if sampled_pos_idx_mask.sum() > 0:
                ...
                localization_loss = torch.tensor(0.0, device=box_transform_pred.device)
...
class ROIHead(nn.Module):
    ...
    def assign_target_to_proposals(self, proposals, gt_boxes, gt_labels):
        ...
        if gt_boxes.numel() == 0:
          ...
          matched_gt_boxes = torch.zeros((proposals.shape[0], 4), dtype=torch.float32, device=proposals.device)
          return labels, matched_gt_boxes
    ...
    def forward(self, image, target=None):
        ...
        if self.training:
            ...
            if not bboxes or len(bboxes[0]) == 0:
              target['bboxes'] = torch.zeros((1, 0, 4), dtype=torch.float32, device=image.device)
            else:
              if isinstance(bboxes, list):
                bboxes = torch.stack(bboxes)
              if bboxes.ndim == 2:
                bboxes = bboxes.unsqueeze(0)
              image, bboxes = self.normalize_resize_image_and_boxes(image, bboxes)
